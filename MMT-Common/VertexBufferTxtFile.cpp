...
void VertexBufferTxtFile::outputVB0File(std::wstring outputFileName) {
    ...
    if (file.is_open()) {
        file << "stride: " << MMTString::ToByteString(Stride) << "\n";
        file << "first vertex: 0" << "\n";
        file << "vertex count: " << MMTString::ToByteString(VertexCount) << "\n";
        file << "topology: " << MMTString::ToByteString(Topology) << "\n";
        //接下来开始输出element列表
        int elementListIndex = 0;
        int AlignedByteOffset = 0;
        for (std::string elementName : realElementList) {
            file << "element[" << elementListIndex << "]:" << "\n";
            D3D11Element elementObject = this->d3d11GameType.ElementNameD3D11ElementMap[elementName];

            file << "  SemanticName: " << elementObject.SemanticName << "\n";
            file << "  SemanticIndex: " << elementObject.SemanticIndex << "\n";
            file << "  Format: " + elementObject.Format << "\n";
            file << "  InputSlot: " << elementObject.InputSlot << "\n";
            file << "  AlignedByteOffset: " << AlignedByteOffset << "\n";
            file << "  InputSlotClass: " << elementObject.InputSlotClass << "\n";
            file << "  InstanceDataStepRate: " << elementObject.InstanceDataStepRate << "\n";
        ...
        for (const auto& pair : IndexVertexDataLineListMap) {
            ...
            for (std::string elementName : realElementList) {
                ...
                VertexDataLine vertexDataLine = elementNameVertexDataLineMap[MMTString::ToWideString(elementName)];
                file << "vb0[" << MMTString::ToByteString(vertexDataLine.Index) << "]";
                //alignedOffset设置为三位数
                std::wstringstream offsetStrStream{};
                offsetStrStream << std::setfill(L'0') << std::setw(3) << AlignedByteOffset;
                file << "+" << MMTString::ToByteString(offsetStrStream.str()) << " " << elementName << ": " << MMTString::ToByteString(vertexDataLine.ElementValue) << "\n";
                AlignedByteOffset = AlignedByteOffset + elementObject.ByteWidth;
            }
            file << "\n";
...
std::vector<D3D11Element> VertexBufferTxtFile::parseShowElementList(const std::wstring& filename) {
    ...
    std::wstring vbSlotNumber = MMTString::GetFileNameFromFilePath(filename).substr(9, 1);
        ...
        if (inElementLine && !inVertexData) {
            ...
            if (colonPos != std::wstring::npos) {
                ...
                if (boost::algorithm::ends_with(key, "semanticname")) {
                    ...
                    d3d11Element.ExtractSlot = "vb" + MMTString::ToByteString(vbSlotNumber);
                    //理论上这个应该是最后一个
                    //LOG.LogOutput(L"Read vbSlotNumber: " + d3d11Element.InputSlot);
                    if (d3d11Element.InputSlot == MMTString::ToByteString(vbSlotNumber)) {
            ...
            if (line.empty() && !lineBefore.empty()) {
                ...
                if (!vertexDataLineList.empty()) {
                    ...
                    for (size_t i = 0; i < vertexDataLineList.size(); i++)
                    {
                        ...
                        int number = elementValueNumberMap[vertexDataLine.ElementValue];
                        if (number == 1) {
                            tmpElementList.push_back(MMTString::ToByteString(vertexDataLine.ElementName));
    ...
    for (std::wstring line : lines) {
        if (!startVertexData) {
            ...
            if (line.empty() && !lineBefore.empty()) {
                ...
                if (boost::algorithm::starts_with(line, vb0Str)) {
                    VertexDataLine dataLine(line);
                    if (boost::algorithm::any_of_equal(this->realElementList, MMTString::ToByteString(dataLine.ElementName))) {
