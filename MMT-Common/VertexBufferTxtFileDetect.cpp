...
VertexBufferDetect::VertexBufferDetect(std::wstring VBFilePath) {
	...
	std::wstring txtFileName = MMTString::GetFileNameFromFilePath(VBFilePath);
	...
	while (std::getline(file, line)) {
		...
		if (boost::algorithm::starts_with(line, "stride")) {
			...
			topology = MMTString::ToByteString(valueStr);
	...
	std::wstring txtFileFolderPath = MMTString::GetFolderPathFromFilePath(VBFilePath);
	std::wstring bufFilePath = txtFileFolderPath + L"/" + txtFileName.substr(0, txtFileName.length() - 4) + L".buf";
	// 确保获取到步长
	...
	int fileSize = MMTFile::GetFileSize(bufFilePath);
	...
	//LOG.NewLine();
	uint64_t fileRealSize = MMTFile::GetRealFileSize_NullTerminated(bufFilePath);
