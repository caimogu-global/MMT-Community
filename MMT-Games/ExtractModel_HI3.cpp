...
void Functions_HI3::ExtractModel() {
    ...
    for (const auto& DrawIB_Pair : G.DrawIB_ExtractConfig_Map) {
        ...
        for (std::wstring FileName : FAData.FrameAnalysisFileNameList) {
        ...
        std::map<uint32_t, std::wstring> firstIndexFileNameMap = FAData.Get_MatchFirstIndex_IBFileName_Map(DrawIB);
