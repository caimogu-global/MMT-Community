...
void Functions_WW11::ExtractModel() {
    for (const auto& pair : G.DrawIB_ExtractConfig_Map) {
        ...
        if (extractConfig.GameType == L"Auto") {
            ...
            for (const auto& pair : GameTypeName_D3d11GameType_Map) {
                ...
                // 从vb0中读取到顶点数量然后在下面的过程中，UE4的VB0一般固定为POSITION长度固定为12
                ...
                for (std::wstring filename : FrameAnalyseFileNameList) {
                    ...
                    //这里Index必须去log.txt中查询，满足出现OMGetRenderTargetsAndUnorderedAccessViews才行
                    DrawCall drawCall = FALog.Index_DrawCall_Map[ibFileData.Index];
                    bool findPsSetShaderResource = false;
                    for (std::wstring ApiCall : drawCall.APICallStringList) {
                        ...
                        if (ApiCallLower.find(L"pssetshaderresources") != std::wstring::npos) {
                            findPsSetShaderResource = true;
                        }
                    }

                    if (!findPsSetShaderResource) {
                        LOG.Info(L"该Index未找到PsSetShaderResources调用，不属于有效Index，自动跳过。");
                        continue;
                    }
        ...
        ExtractModel_FromBuffer_WW11(DrawIB, MatchedGameType);
...
void Functions_WW11::ExtractModel_FromBuffer_WW11(std::wstring DrawIB, std::wstring GameType) {
    FrameAnalysisData FAData = G.GetFrameAnalysisData();
    FrameAnalysisLog FALog = G.GetFrameAnalysisLog();
