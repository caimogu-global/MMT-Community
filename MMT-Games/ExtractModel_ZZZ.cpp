...
void Functions_ZZZ::ExtractModel() {
    ...
    for (const auto& pair : G.DrawIB_ExtractConfig_Map) {
        ...
        for (std::string Category : d3d11GameType.OrderedCategoryNameList) {
            ...
            if (G.GameName == L"ZZZ" || G.GameName == L"GI") {
                // 这里我们还在循环里，会执行三次，所以这里我们使用赋值而不是追加
                // 全面切换到基于Buffer的提取是很重要的事情，不能再用自动类型探测了。
                // 这意味着崩坏三、原神、崩坏星穹铁道都需要重新搞数据类型。
                //LOG.Info("Use ZZZ Type:");
                //for (std::string ElementName: d3d11GameType.OrderedFullElementList) {
                //    LOG.Info(ElementName);
                //}
                //LOG.NewLine();
                //LOG.Info("What is in ElementList? :");
