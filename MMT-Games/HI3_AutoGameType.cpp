...
std::wstring Functions_HI3::AutoGameType(std::wstring DrawIB) {
	...
	for (std::string GameTypeName : Ordered_GPU_Object_D3D11GameTypeList) {
		...
		if (pointlistIndexList.size() == 0 && d3d11GameType.GPUPreSkinning == true) {
			LOG.Info(L"GameType: " + MMTString::ToWideString(d3d11GameType.GameType) + L" use GPU-PreSkinning,but can't find any pointlist file in FrameAnalysisFolder , so skip this.");
			LOG.NewLine();
			continue;
		}
		...
		for (std::wstring TrianglelistIndex : trianglelistIndexList) {
			...
			uint64_t VertexNumber = vbFileDetect.fileBufferVertexCount;
			LOG.Info(L"TrianglelistIndex:" + TrianglelistIndex + L" VertexNumber (File Buffer):" + std::to_wstring(VertexNumber));
		...
		for (const auto& categoryPair : d3d11GameType.CategorySlotMap) {
			...
			if (CategoryStride != FileStride) {
				...
				//崩坏三中，只有COLOR值在Texcoord槽位时用得上
				if (CategoryName == "Texcoord" && d3d11GameType.ElementNameD3D11ElementMap["COLOR"].Category == "Texcoord") {
					...
					//崩坏三中没必要判断，因为崩坏三的特性就是用到的时候再提交上去渲染，不是很规范的用法。
