...
void Functions_HSR::GenerateMod() {
    LOG.Info("Executing: GenerateMod_HSR");

    IniBuilder HSRIniBuilder;
    ...
    for (const auto& pair : G.DrawIB_ExtractConfig_Map) {
        ...
        if (d3d11GameType.GPUPreSkinning) {
            ...
            HSRIniBuilder.AppendSection(TextureOverrideVBSection);
        }
        ...
        HSRIniBuilder.AppendSection(IBSkipSection);
        ...
        HSRIniBuilder.AppendSection(TextureOverrideIBSection);

        //5.写出VBResource部分
        IniSection TextureOverrideVBSection(IniSectionType::TextureOverrideVB);
        for (std::string categoryName : d3d11GameType.OrderedCategoryNameList) {
            TextureOverrideVBSection.Append(L"[Resource" + drawIBConfig.DrawIB + MMTString::ToWideString(categoryName) + L"]");
            TextureOverrideVBSection.Append(L"type = Buffer");
            ...
            if (categoryName == "Blend" && d3d11GameType.PatchBLENDWEIGHTS) {
                ...
                TextureOverrideVBSection.Append(L"stride = " + std::to_wstring(finalBlendStride));

            }
            else {
                TextureOverrideVBSection.Append(L"stride = " + std::to_wstring(d3d11GameType.CategoryStrideMap[categoryName]));

            }
            //vb文件的文件名
            TextureOverrideVBSection.Append(L"filename = Buffer/" + drawIBConfig.DrawIB + MMTString::ToWideString(categoryName) + L".buf");
            TextureOverrideVBSection.NewLine();
        }
        HSRIniBuilder.AppendSection(TextureOverrideVBSection);
        ...
        HSRIniBuilder.AppendSection(ResourceIBSection);
        ...
        if (!drawIBConfig.ForbidAutoTexture && !drawIBConfig.UseHashTexture) {
            ...
            HSRIniBuilder.AppendSection(ResourceTextureSection);
        }
            ...
            for (const auto& DrawIBPair : G.DrawIB_ExtractConfig_Map) {
                ...
                for (const auto& TexturePair : Hash_TextureFileNameMap) {
                    ...
                    if (std::filesystem::exists(FilePath)) {
                        ...
                        HSRIniBuilder.AppendSection(ResourceSection);
                        ...
                        HSRIniBuilder.AppendSection(TextureOverrideSection);
    ...
    HSRIniBuilder.AppendSection(BufferUtils::GetCreditInfoIniSection());

    HSRIniBuilder.SaveToFile(G.TimeOutputFolder + L"Config.ini");
